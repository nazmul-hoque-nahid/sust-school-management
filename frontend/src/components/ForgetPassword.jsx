...
const ForgetPassword = ({ onClose }) => {
  ...
  console.log('ForgetPassword modal rendered');

  useEffect(() => {
    console.log('ForgetPassword modal mounted');
    return () => console.log('ForgetPassword modal unmounted');
  }, []);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all duration-300 scale-80">
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="forget-email" className="block text-sm font-semibold text-gray-700 mb-2">
                Email Address
              </label>
            ...
            {message && (
              <div className={`p-3 rounded-lg ${message.includes('sent') ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                <div className="flex items-center">
                  <svg className={`w-5 h-5 mr-2 ${message.includes('sent') ? 'text-green-500' : 'text-red-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={message.includes('sent') ? "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" : "M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"} />
                  </svg>
